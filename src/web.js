...
function page(title,body,{wide=false}={}){
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="dark"><meta name="description" content="Misfit Mafia YouTube Bot verifies eligible YouTube channel memberships and assigns the corresponding Discord roles."><title>${esc(title)}</title>
  <style>
  ...
  *{box-sizing:border-box}html{background:var(--bg);color-scheme:dark}body{font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:radial-gradient(circle at 50% -10%,#291014 0,#0b0b0c 35%,#060607 72%);color:var(--text);margin:0;min-height:100vh}
  ...
  a{color:inherit}.wrap{max-width:${wide?'1220':'1080'}px;margin:0 auto;padding:28px 18px 54px;position:relative}.topnav{display:flex;justify-content:space-between;align-items:center;gap:16px;padding:10px 0 28px}.brand{display:flex;align-items:center;gap:12px;text-decoration:none}.seal{width:42px;height:42px;border:1px solid #55585f;border-radius:50%;display:grid;place-items:center;background:#111;color:#ddd;font-weight:900;letter-spacing:.08em;box-shadow:inset 0 0 0 4px #0a0a0b}.brand strong{display:block;letter-spacing:.08em;text-transform:uppercase}.brand small{display:block;color:var(--soft);margin-top:2px}.navlinks{display:flex;gap:8px;flex-wrap:wrap}
  .hero{border:1px solid #383a40;border-radius:24px;padding:42px;background:linear-gradient(145deg,rgba(158,23,29,.17),rgba(17,18,20,.96) 42%,rgba(9,9,10,.98));position:relative;overflow:hidden;margin-bottom:20px}.hero:after{content:"MISFIT MAFIA";position:absolute;right:-18px;bottom:-22px;font-size:70px;font-weight:900;letter-spacing:.08em;color:rgba(255,255,255,.025);white-space:nowrap}.kicker{text-transform:uppercase;letter-spacing:.19em;color:#d0d1d4;font-size:12px;font-weight:800}.hero h1{font-size:clamp(34px,7vw,68px);line-height:.95;margin:10px 0 16px;max-width:800px;text-transform:uppercase;letter-spacing:-.035em}.hero p{max-width:760px;color:#c5c6ca;font-size:18px;line-height:1.6;margin:0 0 24px}.hero-actions,.row{display:flex;gap:10px;flex-wrap:wrap;align-items:center}.section-title{margin:34px 0 12px}.section-title h2{margin:0 0 5px}.section-title p{margin:0;color:var(--soft)}
  .grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.grid.two{grid-template-columns:repeat(2,minmax(0,1fr))}.card{background:linear-gradient(180deg,#151619,#101113);border:1px solid var(--line);border-radius:18px;padding:20px;margin-bottom:16px}.card h1,.card h2,.card h3{margin-top:0}.card p,.legal li{line-height:1.65}.choice{min-height:220px;display:flex;flex-direction:column}.choice .icon{font-size:28px;margin-bottom:18px}.choice p{color:var(--soft);line-height:1.55;flex:1}.choice.viewer{border-color:#603238}.choice.creator{border-color:#494c52}.choice.admin{border-color:#494c52}.badge{display:inline-flex;width:max-content;border:1px solid #4a4c52;border-radius:999px;padding:5px 9px;color:#d1d2d5;font-size:12px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:12px}
  ...
  </style></head><body><div class="wrap"><nav class="topnav"><a class="brand" href="/"><span class="seal">MM</span><span><strong>Misfit Mafia YouTube Bot</strong><small>YouTube Membership Verification</small></span></a><div class="navlinks"><a class="btn ghost" href="/privacy">Privacy</a><a class="btn ghost" href="/terms">Terms</a><a class="btn ghost" href="/creator">Creator Login</a><a class="btn ghost" href="/admin">Admin Login</a></div></nav>${body}<div class="footer"><strong>Misfit Mafia YouTube Bot</strong> • YouTube membership verification for connected Discord communities<br><a href="/privacy">Privacy Policy</a><a href="/terms">Terms of Service</a></div></div></body></html>`;
}
...
app.get('/health',(_req,res)=>res.json({ok:true,service:'verification-bot',version:'2.2.0'}));

app.get('/',(_req,res)=>res.send(page('Misfit Mafia YouTube Bot',`
<section class="hero"><div class="kicker">Official membership verification</div><h1>Misfit Mafia YouTube Bot</h1><p>Misfit Mafia YouTube Bot connects a Discord member's linked YouTube identity with participating YouTube creator memberships. It checks whether an eligible membership is active and uses the membership level to assign or maintain the appropriate Discord role. You can read about the service here without signing in; authentication is only required when you choose to verify a membership or access a creator or administrator portal.</p><div class="hero-actions"><a class="btn" href="/verify?mode=verify">Verify Membership</a><a class="btn gray" href="/verify?mode=status">Check Membership</a><a class="btn ghost" href="/privacy">Read Privacy Policy</a></div></section>
<div class="section-title"><h2>How it works</h2><p>The bot uses only the account and membership information needed to provide YouTube-to-Discord membership verification.</p></div>
<section class="grid">
  <article class="card choice viewer"><span class="badge">Viewers</span><div class="icon">◆</div><h2>Verify membership</h2><p>Connect with Discord so the bot can identify your Discord account and its connected YouTube identity. The system compares that YouTube identity with active memberships from participating creator channels and assigns the matching Discord role.</p><div class="row"><a class="btn" href="/verify?mode=verify">Verify Membership</a><a class="btn gray" href="/verify?mode=status">Check Membership</a></div></article>
  <article class="card choice creator"><span class="badge">Creators</span><div class="icon">♛</div><h2>Creator access</h2><p>Participating creators authorize the YouTube channel they own so the bot can read membership levels and current channel-member information required for membership verification.</p><a class="btn gray" href="/creator">Creator Login</a></article>
  <article class="card choice admin"><span class="badge">Administration</span><div class="icon">✦</div><h2>Admin control</h2><p>Authorized administrators manage participating creators, Discord role mappings, grace periods, audits, synchronization, and the Discord verification panel.</p><a class="btn gray" href="/admin">Admin Login</a></article>
</section>
<div class="card"><h2>Privacy and account data</h2><p class="muted">The service does not sell Google or Discord account information. Google/YouTube data is used only to provide membership verification and related Discord-role functionality. Review the full policy before connecting an account.</p><div class="row"><a class="btn gray" href="/privacy">Privacy Policy</a><a class="btn ghost" href="/terms">Terms of Service</a></div></div>`)));

app.get('/privacy',(_req,res)=>res.send(page('Privacy Policy — Misfit Mafia YouTube Bot',`
<main class="legal"><div class="card"><span class="badge">Public policy</span><h1>Privacy Policy</h1><p class="updated">Effective: August 7, 2026</p><p>This Privacy Policy explains how <strong>Misfit Mafia YouTube Bot</strong> handles information when users verify YouTube memberships for Discord roles and when participating YouTube creators connect their channels.</p>
<h2>Information we process</h2><ul><li><strong>Discord information:</strong> Discord user ID, display name or username, and the YouTube connection associated with the Discord account when a user chooses to verify.</li><li><strong>YouTube/Google information:</strong> YouTube channel identifiers, channel names, membership levels, and current membership information needed to determine whether an eligible membership is active and which Discord role applies.</li><li><strong>Creator authorization:</strong> participating creators may authorize Google OAuth access. OAuth access and refresh tokens may be stored so the service can perform ongoing membership synchronization for that creator's channel.</li><li><strong>Operational information:</strong> role mappings, verification status, grace-period records, connection timestamps, and other records necessary to operate the verification service.</li></ul>
<h2>How information is used</h2><p>Information is used only to authenticate accounts, connect Discord users with their linked YouTube identity, determine eligible YouTube membership status and level, assign or remove corresponding Discord roles, maintain grace periods, perform membership synchronization, troubleshoot the service, and protect administrative access.</p>
<h2>Google API data</h2><p>Misfit Mafia YouTube Bot's use and transfer to any other app of information received from Google APIs will adhere to the <a href="https://developers.google.com/terms/api-services-user-data-policy" rel="noopener noreferrer">Google API Services User Data Policy</a>, including the Limited Use requirements.</p><p>Google/YouTube data obtained through OAuth is not sold, used for advertising, or used to build advertising profiles. It is used only for the user-facing membership verification and Discord-role functionality described on this site.</p>
<h2>Sharing</h2><p>We do not sell personal information. Information may be processed by infrastructure providers needed to host the application and database, and relevant verification results may be used within the connected Discord community to assign or remove roles. We do not disclose Google user data to unrelated third parties for advertising or marketing.</p>
<h2>Storage and security</h2><p>Account links, creator authorization records, membership status, and role mappings may be stored in the application's database while needed to operate the service. Reasonable technical measures are used to restrict administrative access. No online service can guarantee absolute security.</p>
<h2>Retention and deletion</h2><p>Information is retained for as long as it is reasonably needed to provide membership verification, maintain a connected creator channel, resolve grace-period status, comply with applicable obligations, or protect the service. Users or creators who want their stored verification or connection information removed can contact a Misfit Mafia administrator through the connected Discord community. Creator access can also be revoked from the Google Account permissions page.</p>
<h2>Your choices</h2><p>Using the public information pages does not require login. Membership verification is optional. Users can choose not to connect or verify an account. Google authorization can be revoked through Google Account settings, and Discord connections can be managed through Discord settings.</p>
<h2>Changes to this policy</h2><p>This policy may be updated when the service, its data practices, or legal requirements change. The effective date above will be updated when material changes are made.</p>
<h2>Contact</h2><p>For privacy questions, access requests, or deletion requests, contact a Misfit Mafia administrator through the Discord community where the verification service is provided.</p><div class="row"><a class="btn" href="/">Back to Home</a><a class="btn gray" href="/terms">Terms of Service</a></div></div></main>`)));

app.get('/terms',(_req,res)=>res.send(page('Terms of Service — Misfit Mafia YouTube Bot',`
<main class="legal"><div class="card"><span class="badge">Public terms</span><h1>Terms of Service</h1><p class="updated">Effective: August 7, 2026</p><p>These Terms govern use of <strong>Misfit Mafia YouTube Bot</strong>, a service that verifies eligible YouTube channel memberships and manages corresponding Discord roles for participating communities.</p>
<h2>Use of the service</h2><p>You may use the service only for legitimate membership verification and related Discord-role functions. You must not attempt to bypass authentication, impersonate another user, interfere with the service, access administrative functions without authorization, or misuse Google, YouTube, or Discord data.</p>
<h2>Account connections</h2><p>Verification may require Discord authentication and a YouTube account connected to Discord. Participating creators may also authorize their YouTube channel through Google OAuth. You are responsible for using accounts you are authorized to access.</p>
<h2>Membership results</h2><p>Membership and role results depend on information made available by YouTube, Discord, participating creators, and configured role mappings. A temporary delay, API limitation, grace period, configuration error, or service outage may affect verification results.</p>
<h2>Availability</h2><p>The service is provided on an as-available basis. Features may be changed, suspended, or discontinued as needed for maintenance, security, platform requirements, or community administration.</p>
<h2>Platform rules</h2><p>Use of the service remains subject to applicable Google, YouTube, and Discord terms and policies. Nothing in these Terms grants rights to those third-party services or content.</p>
<h2>Termination</h2><p>Access may be restricted or removed for abuse, attempts to compromise the service, violations of these Terms, loss of an eligible membership, removal of a participating creator, or administrative/security reasons.</p>
<h2>Privacy</h2><p>How information is collected and used is described in the <a href="/privacy">Misfit Mafia YouTube Bot Privacy Policy</a>.</p>
<h2>Changes</h2><p>These Terms may be updated as the service changes. Continued use after an updated version is posted constitutes acceptance of the revised Terms to the extent permitted by law.</p><div class="row"><a class="btn" href="/">Back to Home</a><a class="btn gray" href="/privacy">Privacy Policy</a></div></div></main>`)));
...
app.get('/creator',async(req,res,next)=>{
  try{
    ...
    if(!channelId){
      return res.send(page('Creator Login',`<div class="card login-card"><span class="badge">Creator portal</span><h1>Creator Login</h1><p class="muted">Sign in with the Google account that owns the memberships-enabled YouTube channel you want connected to Misfit Mafia YouTube Bot.</p><a class="btn wide" href="/creator/login">Continue with YouTube</a><div class="divider"></div><a class="btn ghost wide" href="/">Back to viewer verification</a></div>`));
    }
...
app.get('/admin',async(req,res,next)=>{
  try{
    if(!isAdmin(req)){
      return res.send(page('Admin Login',`<div class="card login-card"><span class="badge">Restricted access</span><h1>Admin Login</h1><p class="muted">Enter the Misfit Mafia YouTube Bot administrator password to open system controls.</p><form method="post" action="/admin/login"><label class="field"><span>Admin password</span><input type="password" name="password" autocomplete="current-password" required></label><button class="btn wide" type="submit">Enter Admin Dashboard</button></form></div>`));
    }
    ...
    res.send(page('Misfit Mafia YouTube Bot Admin',`<div class="card"><div class="row"><div><span class="badge">Administration</span><h1>Misfit Mafia YouTube Bot Admin</h1><p class="muted">Manage creators, roles, grace periods, syncing and the verification panel.</p></div><a class="btn gray right" href="/admin/logout">Logout</a></div></div><div class="grid two"><div class="card"><h2>Connect a Creator</h2><p class="muted">Authorize a memberships-enabled YouTube channel and add it to verification.</p><a class="btn" href="/admin/creator/connect">Connect YouTube Creator</a></div><div class="card"><h2>Post Verification Panel</h2><form method="post" action="/admin/post-panel"><label class="field"><span>Discord channel</span><select name="channel_id" required><option value="">Choose Discord channel</option>${channelOpts}</select></label><button class="btn" type="submit" style="margin-top:10px">Post Panel</button></form></div></div>${creatorCards.join('')||'<div class="card"><h2>No creators connected yet</h2></div>'}`,{wide:true}));
  }catch(e){next(e);}
});
...
app.use((e,_req,res,_next)=>{
  console.error(e);
  res.status(500).send(page('Misfit Mafia YouTube Bot Error',`<div class="card login-card"><span class="badge">System error</span><h1>Something went wrong</h1><p class="bad">${esc(e.message)}</p><div class="row"><a class="btn" href="/">Home</a><a class="btn gray" href="/admin">Admin</a></div></div>`));
});
