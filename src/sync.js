...
export async function syncCreator(creatorChannelId) {
  ...
  auth.setCredentials({ access_token: creator.access_token, refresh_token: creator.refresh_token, expiry_date: creator.token_expiry ? Number(creator.token_expiry) : undefined });
  auth.on('tokens', async tokens => {
    await db.query(`UPDATE creators SET access_token = COALESCE($1, access_token), refresh_token = COALESCE($2, refresh_token), token_expiry = COALESCE($3, token_expiry) WHERE youtube_channel_id = $4`, [tokens.access_token, tokens.refresh_token, tokens.expiry_date, creatorChannelId]);
  });
  ...
  for (const user of linkedUsers) {
    ...
    for (const mapping of mappings) {
      await setMemberRole(user.discord_user_id, mapping.discord_role_id, activeLevel === mapping.youtube_level_id);
      changes++;
    }
  }
